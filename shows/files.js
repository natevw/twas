function (doc, req) {
    ...
    provides("html", function () {
        ...
        var template = "<li><a href='{{db_url}}/{{id}}/{{name}}'>./{{ encName }}</a></li>",
            files = (doc && doc._attachments) ? Object.keys(doc._attachments).map(function (name) {
                return fs.to_html(template, {db_url:db_url, id:doc._id, name:name, encName:encodeURIComponent(name).replace(/%20/g,'+')});
        ...
        return fs.to_html(ddoc.templates.files, {
            ...
            id:(doc && doc._id), revno:(doc && doc._rev), summary:(doc && doc.content.slice(0,255))
        });
    });
}
